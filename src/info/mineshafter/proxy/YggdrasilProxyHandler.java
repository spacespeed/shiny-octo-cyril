...
public class YggdrasilProxyHandler implements HttpProxyHandler, SocksProxyHandler {
	public static String authServer = "localhost:9010";
	...
	public static Pattern UPSERVER_URL = Pattern.compile("http://libraries\\.minecraft\\.net/(.*)");
//	public static Pattern MINECRAFT_URL = Pattern.compile("http://s3\\.amazonaws\\.com/Minecraft\\.Download/(.*)");

	private Map<String, byte[]> skinCache = new Hashtable<String, byte[]>();
	private Map<String, byte[]> cloakCache = new Hashtable<String, byte[]>();

	private YggdrasilImpersonator imp = new YggdrasilImpersonator(authServer);
	...
	public boolean onGET(String url, Map<String, String> headers, InputStream in, OutputStream out) {
		...
		Matcher checkserverMatcher = CHECKSERVER_URL.matcher(url);
		Matcher joinserverMatcher = JOINSERVER_URL.matcher(url);
		Matcher upserverMatcher = UPSERVER_URL.matcher(url);
//		Matcher minecraftMatcher = MINECRAFT_URL.matcher(url);
		...
		if (skinMatcher.matches()) {
			...
			if (skinCache.containsKey(username)) {
				...
				url = "http://" + authServer + "/skin/" + username;
			...
			if (cloakCache.containsKey(username)) {
				...
				url = "http://" + authServer + "/cloak/" + username;
				System.out.println("To: " + url);
				data = getRequest(url);
				System.out.println("Response length: " + data.length);
			...
			String response = imp.checkserver(request[1]);
			sendResponse(out, "text/plain",
					response.getBytes(Charset.forName("utf-8")));
			return true;
		} else if (joinserverMatcher.matches()){
			String[] request = url.split("[?]");
			String response = imp.joinserver(request[1]);
			sendResponse(out, "text/plain",
					response.getBytes(Charset.forName("utf-8")));
			return true;
		} else if (upserverMatcher.matches()) {
			String path = upserverMatcher.group(1);
			url = "http://" + authServer + "/" + path;
			data = getRequest(url);
			sendResponse(out, "application/java-archive", data);
			return true;
		}
